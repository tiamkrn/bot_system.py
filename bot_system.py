...
def extract_data(text):
    ...
    # تمام فاصله‌ها و نقاط‌ها رو حذف کن (فقط برای استخراج)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # بدهی
            ...
            data = load_data(DATA_DIR / 'debt_data.json')
            data.append({'username': username, 'amount': amount, 'timestamp': format_datetime()})
            save_data(DATA_DIR / 'debt_data.json', data)
            response = f"✅ **بدهی**\n👤 {username}\n💰 {amount}"
            await update.message.reply_text(response, parse_mode='Markdown')
            await update.message.delete()
            logger.info(f"✅ بدهی ذخیره شد")
